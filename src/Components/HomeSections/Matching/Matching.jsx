        ...
        <div className="matchingCard-container">
                ...
                {deniedUsers.slice(-1).map((user, index) => (
                    <div key={user?.id || index} className="matchingCard-containerImage">
                        <figure
                            ...
                            style={{
                                backgroundImage: `linear-gradient(to top, rgba(0,0,0,0.5), rgba(0,0,0,0.0)), url(${user?.image || 'default-image-url.jpg'})`,
            ...
            <div className="matchingCard-middle">
                {allUsersData.length > 0 && currentIndex < allUsersData.length ? (
                    <div key={allUsersData[currentIndex]?.id} className="matchingCard-containerImage">
                        <figure
                            className="matchingCard-containerImage"
                            style={{
                                backgroundImage: `linear-gradient(to top, rgba(0,0,0,0.5), rgba(0,0,0,0.0)), url(${allUsersData[currentIndex]?.image || 'default-image-url.jpg'})`,
                                backgroundSize: "cover",
                                backgroundPosition: "center",
                                backgroundRepeat: "no-repeat",
                            }}
                        ></figure>
                        <article className="matchingCard-info">
                            <p className="matchingCard-name">{capitalizeFirstLetter(allUsersData[currentIndex]?.name) || 'Nombre no disponible'}</p>
                            <p className="matchingCard-category">{allUsersData[currentIndex]?.Profession?.name || 'Categoría no disponible'}</p>
                            <article className='matchingCard-buttonsMatching'>
                                <button onClick={handleDenied} className='matchingCard-buttons close'>
                                    <IoIosCloseCircle />
                                </button>
                                <button onClick={handleAccepted} className='matchingCard-buttons check'>
                                    <AiFillCheckCircle />
                                </button>
                            </article>
                        </article>
                ...
                {acceptedUsers.slice(-1).map((user, index) => (
                    <div key={user?.id || index} className="matchingCard-containerImage">
                        ...
                        <p className="matchingCard-name">{capitalizeFirstLetter(user?.name) || 'Nombre no disponible'}</p>
                            <p className="matchingCard-category">{user?.Profession?.name || 'Categoría no disponible'}</p>
                            <article className='matchingCard-buttonsMatching'>
                                <button className='matchingCard-buttons check' style={{fontSize: "3rem"}}>
                                    <AiFillCheckCircle />
                                </button>
                            </article>
                        </article>
